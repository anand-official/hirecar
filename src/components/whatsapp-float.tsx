...
export function WhatsAppFloat({ phone }: WhatsAppFloatProps) {
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-[60] flex items-center gap-3 rounded-full bg-[#25D366] pl-4 pr-5 py-3.5 text-white font-bold shadow-2xl shadow-emerald-600/30 hover:bg-[#1ebe5d] hover:scale-105 transition-all"
    >
      ...
      <span className={`overflow-hidden whitespace-nowrap transition-all duration-300 ${hovered ? "max-w-[140px] opacity-100" : "max-w-0 opacity-0 sm:max-w-[140px] sm:opacity-100"}`}>
        Chat with us
      </span>
    </a>
  );
